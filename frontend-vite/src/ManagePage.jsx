...
function getIndividualName(individual_id, peopleById) {
    let given_name = peopleById.get(individual_id).given_name ?? ``;
    let second_name = peopleById.get(individual_id).second_name ?? ``;
    let third_name = peopleById.get(individual_id).third_name ?? ``;
    let surname = peopleById.get(individual_id).surname ?? ``;
    return (
        `${given_name} ${second_name} ${third_name} ${surname}`
    );
}
...
function IndividualDrawer({open, editId, onClose, onSaved}) {
    ...
    const [form, setForm] = useState({
        given_name: '', second_name: '', third_name: '', surname: '',
        maiden_name: '', sex: '', birth_date: '', birth_place: '',
    ...
    useEffect(() => {
        ...
        fetch(`/api/v1/individuals/${editId}`).then(r => r.json()).then(p => {
            setForm({
        ...
        <aside className={`drawer${open ? ' open' : ''}`}>
            <div className="drawer__header">
                ...
                <div className="drawer__header-actions">
                    {isEdit ? <button className="danger">Person löschen</button> : ''}
            ...
            <div className="drawer__field">
                ...
                <input value={form.birth_date} type='date' onChange={set('birth_date')}/>
            </div>
            <div className="drawer__field">
            ...
            <div className="drawer__field">
                ...
                <input value={form.death_date} type='date' onChange={set('death_date')}/>
            </div>
            <div className="drawer__field">
                <label>Sterbeort:</label>
                <input value={form.death_place} onChange={set('death_place')}/>
            </div>
            <button className="drawer__save" onClick={submit}>Speichern</button>
...
function FamilyDrawer({open, editId, individuals, onClose, onSaved}) {
    ...
    const [form, setForm] = useState({
        partner1_id: '', partner2_id: '', wedding_date: '', children: [],
    });
        ...
        <>
            ...
            {individuals.map(p => (
                <option key={p.id} value={p.id}>
                    {p.given_name} {p.surname ?? ''}{p.birth_date ? ` (${p.birth_date.slice(0,4)})` : ''}
                </option>
            ))}
        </>
    );
    ...
    useEffect(() => {
        if (!isEdit) {
            setForm({ partner1_id: '', partner2_id: '', wedding_date: '', children: [] });
            return;
        }
        fetch(`/api/v1/families/${editId}`).then(r => r.json()).then(f => {
            setForm({
    ...
    return (
        <aside className={`drawer${open ? ' open' : ''}`}>
            <div className="drawer__header">
                ...
                <div className="drawer__header-actions">
                    {isEdit ? <button className="danger">Familie löschen</button> : ''}
                    <button onClick={onClose}>X</button>
                </div>
            </div>
            <div className="drawer__field">
                <label>Ehepartner 1:</label>
                <select value={form.partner1_id} onChange={set('partner1_id')}>{personOptions}</select>
            </div>
            <div className="drawer__field">
                <label>Ehepartner 2:</label>
                <select value={form.partner2_id} onChange={set('partner2_id')}>{personOptions}</select>
            </div>
            <div className="drawer__field">
                <label>Hochzeitsdatum:</label>
                <input value={form.wedding_date} type='date' onChange={set('wedding_date')}/>
            </div>
            <div className="drawer__field">
                <label>Kinder:</label>
                {form.children.map((childId, idx) => (
                    <div className="drawer__child-row" key={idx}>
                        ...
                        <button type='button' onClick={() => removeChild(idx)}>x</button>
                    </div>
                )) }
                <button className="drawer__add" type="button" onClick={addChild}>Kind hinzufügen</button>
            </div>
            <button className="drawer__save" onClick={submit}>Speichern</button>
            <div className="drawer__meta">editId: {editId ?? 'create'}</div>
        </aside>
...
export default function ManagePage() {
  ...
  const [familyDrawer, setFamilyDrawer] = useState(null);
    ...
    async function loadTables() {
        ...
        ppl.sort((a, b) => {
            const givenName = (a.given_name ?? '').localeCompare(
                b.given_name ?? ''
            );
            ...
            return (a.birth_date ?? '').localeCompare(
                b.birth_date ?? ''
            );
        });
        ...
        <div className="manage-page">
            <header>
                ...
                <div className="actions">
                    ...
                    <button onClick={() => setPersonDrawer({})}>Person erstellen</button>
                    <button onClick={() => setFamilyDrawer({})}>Familie erstellen</button>
                </div>
            </header>
            <div className="tables">
                    ...
                    <tbody>
                        {individuals.map(p => (
                            <tr key={p.id}>
                                ...
                                <td>{p.death_date}</td>
                                <td>
                                    <button onClick={() => setPersonDrawer({id: p.id})}>Bearbeiten</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                    ...
                    <tbody>
                        {families.map(fam => (
                            <tr key={fam.id}>
                                ...
                                <td>
                                    <button onClick={() => setFamilyDrawer({id: fam.id})}>Bearbeiten</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                ...
                <IndividualDrawer
                    ...
                    onSaved={() => { setPersonDrawer(null); loadTables(); }}
                />
                <FamilyDrawer
                    open={!!familyDrawer}
                    ...
                    onSaved={() => { setFamilyDrawer(null); loadTables(); }}
                />
            </div>
        </div>
    )
