...
function PersonNode({ data }) {
  return (
    <div className="person-card" style={{ width: CARD_WIDTH, height: CARD_HEIGHT }}>
      ...
      {data.maiden_name ? <div className="person-card__maiden">
        {`geb. ${data.maiden_name}`}
      </div> : ''}
      <Handle type="target" position={Position.Top} id="top" style={{left: CARD_WIDTH/2, right: -CARD_WIDTH/2, top: CARD_HEIGHT/2, bottom: -CARD_HEIGHT/2, hiddenHandle}} />
      <Handle type="source" position={Position.Right} id="right" style={{left: CARD_WIDTH/2, right: -CARD_WIDTH/2, top: CARD_HEIGHT/2, bottom: -CARD_HEIGHT/2, hiddenHandle}} />
      <Handle type="target" position={Position.Left} id="left" style={{left: CARD_WIDTH/2, right: -CARD_WIDTH/2, top: CARD_HEIGHT/2, bottom: -CARD_HEIGHT/2, hiddenHandle}}  />
    </div>
  );
}
...
function UnionNode() {
  return (
    <div style={{width: 1, height: 1, position: 'relative'}}>
      <Handle type='target' position={Position.Top} id="in" style={{ left: 0, top: 0, hiddenHandle}} />
      <Handle type="source" position={Position.Bottom} id="out" style={{ left: 0, top: 0, hiddenHandle}} />
    </div>
  )
}
...
function moveSubtree(left, right, shift) {
  let subtrees = right.index - left.index;
...
function apportion(node, defaultAncestor) {
  ...
  let vInsideRight    = node;                     let sInsideRight    = node.mod;
  let vOutsideRight   = node;                     let sOutsideRight   = node.mod;
  let vInsideLeft     = leftSibling;              let sInsideLeft     = leftSibling.mod;
  let vOutsideLeft    = node.parent.children[0];  let sOutsideLeft    = vOutsideLeft.mod;
  ...
  while (nextRight(vInsideLeft) && nextLeft(vInsideRight)) {
    ...
    let shift = (( vInsideLeft.preX + sInsideLeft )
...
function findRootFamilies(people, families) {
  ...
  return families.filter(fam =>     // filter those families where neither partner1_id and partner2_id are not present in childIds
...
function buildTree(root, people, families, depth = 0) {
  ...
  for (const childId of root.children ?? []) {  // append a child element for each child of the root family

    

    const childsFamily = families.find( // check if child has its own family
      fam => 
        fam.partner1_id === childId ||
        fam.partner2_id === childId
    );

    if (childsFamily) { // skipping leaf children
      // console.log('child has a family', childsFamily.id);
      // console.log("childsfamily:", childsFamily);

      const childTree = buildTree(  // append the childs own tree
                          childsFamily,
                          people,
                          families,
                          depth + 1);
...
function firstWalk(node) {
  if (node.children.length == 0) { // if node has any children

    if (node.leftSibling) { // if node has leftSibling --> separate from it
      node.preX = node.leftSibling.preX + separation(node.leftSibling, node);
    } else { // if no leftSibling start at 0
      node.preX = 0;
    }
  ...
  let midpoint = (node.children[0].preX + node.children[node.children.length - 1].preX) / 2;
  ...
  switch (tree.type) {
    case 'individual': {
      nodes.push({
        id: tree.id,
        position: {
          x: anchorToTopLeftX(tree.x, CARD_WIDTH), y: tree.y
        },
    ...
    case 'union': {
      nodes.push({
        id: tree.id,
        position: {
          x: tree.x, y: tree.y + CARD_HEIGHT / 2
        },
        data: {label: 'union'},
        type: 'union'
      })
      ...
      if (tree.partner1_id) {
        nodes.push({
          ...
          position: {
            x: offsetPartnerLeft(anchorToTopLeftX(tree.x, CARD_WIDTH), PARTNER_GAP, CARD_WIDTH), y: tree.y
          },
      ...
      if (tree.partner2_id) {
        nodes.push({
          ...
          position: {
            x: offsetPartnerRight(anchorToTopLeftX(tree.x, CARD_WIDTH), PARTNER_GAP, CARD_WIDTH), y: tree.y
          },
...
function PersonDetail({ id, families, peopleById, onClose, onSelect }) {
  ...
  useEffect(() => {
    if (id == null) return;
    fetch(`/api/v1/individuals/${id}`).then(r => r.json()).then(setPerson);
  }, [id]);
  ...
  return (
    <aside className="person-detail open">
      <button className="person-detail__close" onClick={onClose}>×</button>
      <img
        className="person-detail__portrait"
        src={person.portrait_url || placeholderAvatar}
      />
      <h2>{person.given_name} {person.second_name ?? ''} {person.surname}</h2>
      {person.maiden_name && (
        <div className="person-detail__maiden">geb. {person?.maiden_name}</div>
      )}
      <section>
        <div className="person-detail__label">Lebensdaten</div>
        {person.birth_date && <div>
          <span className="person-detail__mark">∗</span>
          <span>{person.birth_date}{person.birth_place ? `, ${person.birth_place}` : ''}</span>
        </div>}
        {person.death_date && <div>
          <span className="person-detail__mark">†</span>
          <span>{person.death_date}{person.death_place ? `, ${person.death_place}` : ''}</span>
        </div>}
      </section>
      <section>
        ...
        <>
          ...
          <div>
            {parents.map(pid => {
              ...
              if (!p) return null;
              return (
                <button key={pid} className="relative-button" onClick={() => onSelect(pid)}>{p.given_name} {p.surname}</button>
              )
            })}
          </div>
        </>}
        ...
        <>
          ...
          <div>
            {siblings.map(sid => {
              ...
              if (!s) return null;
              return (
                <button key={sid} className="relative-button" onClick={() => onSelect(sid)}>{s.given_name} {s.surname}</button>
              )
            })}
          </div>
        </>  }
        ...
        <>
          ...
          <div>
            {children.map(cid => {
              ...
              if (!c) return null;
              return (
                <button key={cid} className="relative-button" onClick={() => onSelect(cid)}>{c.given_name} {c.surname}</button>
              )
            })}
          </div>
        </>}
      ...
      {roots.map(root => (
        <li key={root.id}>
          <Link to={`/tree/${root.id}`}>
            {nameOf(peopleById, root.partner1_id)} &amp; {nameOf(peopleById, root.partner2_id)}
          </Link>
        </li>
      ))}
    </ul>
