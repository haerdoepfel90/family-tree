...
export default function DetailPage() {
    ...
    useEffect(() => {
        fetch("/api/v1/families").then(r => r.json()).then(setFamilies);
    }, [])

    useEffect(() => {
        fetch("/api/v1/individuals").then(r => r.json()).then(setIndividuals);
    }, []);

    useEffect(() => {
        if (id == null) return;
        fetch(`/api/v1/individuals/${id}/documents`).then(r => r.json()).then(setDocuments);
    }, [id]);
            ...
            <div className="detail-page__body">
                <div className="detail-page__card detail-page__person-card">
                    ...
                    <div>
                        ...
                        {person.birth_date && (
                            <div>* {person.birth_date}{person.birth_place ? `, ${person.birth_place}` : ''}</div>
                        )}
                        {person.death_date && (
                            <div>† {person.death_date}{person.death_place ? `, ${person.death_place}` : ''}</div>
                        )}
                    </div>
                </div>
                <div className="detail-page__card-row">
                    <div className="detail-page__card">
                        <div className="detail-page__card-title">
                            Familie
                        </div>
                        {relatives.parents.length > 0 &&
                            <div>
                                ...
                                {relatives.parents.map(pid => (
                                    <Link style={{display: 'block'}} to={`/detail/${pid}`} >{formatName(getPersonByID(pid, peopleById), 'normal')}</Link>
                                ))}
                            </ div>
                        }
                        {relatives.siblings.length > 0 &&
                            <div>
                                ...
                                {relatives.siblings.map(sid => (
                                    <Link style={{display: 'block'}} to={`/detail/${sid}`} >{formatName(getPersonByID(sid, peopleById), 'normal')}</Link>
                                ))}
                            </div>
                        }
                        ...
                        {relatives.children.length > 0 &&
                            <div>
                                ...
                                {relatives.children.map(cid => (
                                    <Link style={{display: 'block'}} to={`/detail/${cid}`} >{formatName(getPersonByID(cid, peopleById), 'normal')}</Link>
                                ))}
                            </div>
                        }
                    </div>
                    <div className="detail-page__card">
                        ...
                        {person.birth_date &&
                            <div style={{display: "flex", alignItems: "baseline", gap: "8px"}}>
                                <div className="detail-page__label" style={{minWidth: "80px"}}>Geboren</div>
                                <div>{person.birth_date}{person.birth_place ? `, ${person.birth_place}` : ''}</div>
                            </ div>
                        }
                        {person.death_date &&
                            <div style={{display: "flex", alignItems: "baseline", gap: "8px"}}>
                                <div className="detail-page__label" style={{minWidth: "80px"}}>Gestorben</div>
                                <div>{person.death_date}{person.death_place ? `, ${person.death_place}` : ''}</div>
                            </ div>
                        }
                    </div>
